...
void deploy(String deployEnvironment) {
	...
	sh """
		...
		marathon-config-producer deploy/marathon single hydra-${deployEnvironment} --template-keys BUILD_NUMBER=${env.BUILD_NUMBER} -o hydra-service-${deployEnvironment}.json
...
pipeline {
	...
	stages {
		...
		stage("docker build") {
			steps {
				script {
					def image = docker.build("docker-io.dbc.dk/hydra-service:${env.BUILD_NUMBER}")
					image.push()
				}
			}
		}
